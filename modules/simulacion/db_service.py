...
class SimulacionDBService:
    ...
    async def get_report_metricas_generales_batch(self, conn, filters: Dict[str, Any], cats: Dict) -> List[Dict[str, Any]]:
        """
        Versión batch de get_report_metricas_generales_row.
        Retorna 1 fila por usuario activo (GROUP BY responsable_simulacion_id).
        NULLs excluidos explícitamente (12 registros en rango 2025 — Check 1 validado vía MCP).
        """
        where_clause, params = self._build_report_where_clause(filters)
    ...
    async def get_report_metricas_tech_batch(self, conn, filters: Dict[str, Any], cats: Dict) -> List[Dict[str, Any]]:
        """
        Versión batch de get_report_metricas_tech.
        Retorna filas por (responsable_simulacion_id, tecnologia).
        Solo tecnologías con datos para cada usuario (equivalente funcional del LEFT JOIN por usuario).
        """
        where_clause, params = self._build_report_where_clause(filters)
        where_batch = where_clause + " AND o.responsable_simulacion_id IS NOT NULL"
    ...
    async def get_report_tabla_contabilizacion_batch(self, conn, filters: Dict[str, Any], cats: Dict) -> List[Dict[str, Any]]:
        """
        Versión batch de get_report_tabla_contabilizacion.
        Retorna filas por (responsable_simulacion_id, tipo_solicitud).
        Solo tipos con datos para cada usuario (HAVING COUNT > 0, igual que la versión individual).
        """
        where_clause, params = self._build_report_where_clause(filters)
        where_batch = where_clause + " AND o.responsable_simulacion_id IS NOT NULL"
    ...
    async def get_report_tiempo_promedio_tipo_batch(self, conn, filters: Dict[str, Any], cats: Dict) -> List[Dict[str, Any]]:
        """
        Versión batch de get_report_tiempo_promedio_tipo.
        Retorna filas por (responsable_simulacion_id, tipo_solicitud).
        """
        where_clause, params = self._build_report_where_clause(filters)
        where_batch = where_clause + " AND o.responsable_simulacion_id IS NOT NULL"
    ...
    async def get_report_tiempo_promedio_global_batch(self, conn, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Versión batch de get_report_tiempo_promedio_global.
        PERCENTILE_CONT(0.95) calculado por usuario via CTE separada (no soporta OVER () en batch).
        Validado vía MCP Check 4: idéntico al per-usuario con tolerancia 0.01 días.
        """
        where_clause, params = self._build_report_where_clause(filters)
        where_batch = where_clause + " AND o.responsable_simulacion_id IS NOT NULL"
    ...
    async def get_report_motivo_retrabajo_batch(self, conn, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Versión batch de get_report_motivo_retrabajo.
        DISTINCT ON retorna el motivo más frecuente por usuario.
        """
        where_clause, params = self._build_report_where_clause(filters)
        where_batch = where_clause + " AND o.responsable_simulacion_id IS NOT NULL"
