...
"""Capa de acceso a datos (SQL puro) de las métricas operativas de Simulación.

Recibe la conexión asyncpg y devuelve los dataclasses definidos en
`metrics_models`. La lógica de negocio sin SQL (cuellos de botella) vive en
`MetricsService`.
"""
...
class MetricsDBService:
    ...
    async def get_tiempo_por_estatus(
        ...
        query = f"""
            ...
            en_rango AS (
                -- orden 1..4: pipeline de simulacion (Pendiente, En Proceso, En Revision,
                -- Comentarios Recibidos). Se excluyen estatus comerciales post-entrega
                -- (orden 6 Monitoreo de Cotizacion, 7 Ganada) para no contar espera
                -- comercial como cuello de botella de simulacion.
                SELECT
                    estatus,
    ...
    async def get_metricas_ciclo_revision(
        ...
        VENTANA (intencional, no es bug): aquí se mide el esfuerzo TOTAL del ciclo
        (todos los intervalos en revisión/retrabajo de la OP, sin topar en la
        entrega) → "cuánto tarda en cerrarse la oportunidad". Es DISTINTO a
        get_comparativo_sla_ajustado, que sí topa en la entrega porque mide el
        SLA por solicitud. Las dos lecturas conviven a propósito y están
        ...
        query = f"""
            ...
            tiempo_revision AS (
                -- orden=3: En Revisión (días naturales, incluye fines de semana).
                -- Total del ciclo, SIN topar en la entrega (ver docstring): mide
                -- esfuerzo total de Dirección, no el SLA por solicitud.
            ...
            tiempo_retrabajo AS (
                -- orden=4: Comentarios Recibidos (días naturales, incluye fines de semana)
                SELECT id_oportunidad,
                       SUM(EXTRACT(EPOCH FROM (fin - inicio)) / 86400.0) AS dias
                FROM transiciones
            ...
            SELECT
                ...
                -- Promedio por oportunidad entregada: las que no reingresaron a
                -- revisión cuentan como 0 rondas (COALESCE antes del AVG).
                COALESCE(AVG(COALESCE(r.num_rondas, 0)::float), 0) AS rondas_promedio
            FROM entregadas e
    ...
    async def get_comparativo_sla_ajustado(
        ...
        query = f"""
            ...
            entregas_efectivas AS (
                -- Ancla único de cierre por oportunidad: una sola fecha de entrega
                -- compartida por el total y por el descuento de revisión.
                SELECT
                    er.id_oportunidad,
            ...
            tiempo_revision AS (
                -- Tiempo en revisión (orden=3) en días naturales, TOPADO en la entrega
                -- (LEAST con fecha_entrega): solo descuenta del SLA la revisión previa
                -- a la entrega. Difiere a propósito de get_metricas_ciclo_revision, que
                -- suma el total del ciclo sin topar.
                SELECT
                    t.id_oportunidad,
                    SUM(
                        EXTRACT(EPOCH FROM (
...
def get_metrics_db_service() -> MetricsDBService:
    return MetricsDBService()
